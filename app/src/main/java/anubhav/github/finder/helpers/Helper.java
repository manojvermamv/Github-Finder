...
public class Helper {
    ...
    public static void backupData(final Context ctx) {
        showPositiveNegativeActionsDialog(ctx,
                ...
                (dialog, which) -> {
                    Gson gson = new Gson();
                    /*AppRepository repo = MyApp.getAppInstance().repository;
                    try {
    ...
    public static void restoreData(final Context ctx) {
        showPositiveNegativeActionsDialog(ctx,
                ...
                (dialog, which) -> {
                    Gson gson = new Gson();
                    /*AppRepository repo = MyApp.appInstance.repository;
                    try {
                        MqttClient.subscribe(BACKUP_MQTT_TOPIC, (msg) -> {
                            ...
                            if (deserializedControls != null && deserializedControls.length > 0) {
                                ...
                                Log.d(MyApp.TAG, "Restored " + recordIds.size() + " controls");
    ...
    public static Bitmap getCustomIconBitmap(int iconId, boolean adaptToTheme) {
        ...
        Drawable customIconDrawable = MyApp.getAppInstance().getIconPack().getIconDrawable(iconId, MyApp.getAppInstance().getIconPackLoader().getDrawableLoader());
        if (customIconDrawable != null) {
            ...
            customIconDrawable.setColorFilter(new BlendModeColorFilter(adaptToTheme ? Helper.getThemePrimaryColorInverted(MyApp.getAppContext()) : Color.WHITE, BlendMode.SRC_ATOP));
